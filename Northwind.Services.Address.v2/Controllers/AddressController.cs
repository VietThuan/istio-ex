...
namespace NorthWind.Services.Address.v2.Controllers
{
    ...
    public class AddressController : Controller
    {
        ...
        {
            return new Response("address/validate", 2);
        }

        [HttpPost("validate")]
        public Response<ValidationResponse> Post([FromBody] ValidationRequest request)
        {
            var result = new Response<ValidationResponse>("address/validate", 2);

            result.Data = new ValidationResponse();

            try
            {
                result.Data.Status = !string.IsNullOrEmpty(request.Address) && request.Country.ToLower()
                    .Equals("australia");
